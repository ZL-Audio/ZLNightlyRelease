...
def upload_gitee_asset(gitee_release_id, asset):
    """Downloads an asset from GitHub to local disk, then uploads it to Gitee with retries."""
    asset_name = asset['name']
    asset_url = asset['url']
    
    MAX_RETRIES = 8
    TIMEOUT = 45 # Kills the request if the network hangs for 45 seconds
    
    print(f"    - Handling asset: {asset_name}")
    
    # Use a temporary directory to automatically clean up the file after upload
    with tempfile.TemporaryDirectory() as temp_dir:
        local_file_path = os.path.join(temp_dir, asset_name)
        
        # --- PHASE 1: Download to Local Disk ---
        print(f"      - Downloading {asset_name} from GitHub to local disk...")
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                with requests.get(asset_url, headers=headers, stream=True, timeout=TIMEOUT) as download_response:
                    download_response.raise_for_status()
                    
                    # Write the file to the disk in 8KB chunks
                ...
                if attempt == MAX_RETRIES:
                    print(f"      - Giving up on downloading {asset_name}.", file=sys.stderr)
                    return # Skip to the next asset
                time.sleep(2 ** attempt) # Wait 2s, then 4s, then retry
        
        # --- PHASE 2: Upload to Gitee ---
        print(f"      - Uploading {asset_name} to Gitee...")
        upload_url = f"{GITEE_API_URL}/releases/{gitee_release_id}/attach_files"
        params = {"access_token": GITEE_TOKEN}
        
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                # Open the locally saved file and stream it to Gitee
                with open(local_file_path, 'rb') as f:
                    files = {'file': (asset_name, f, asset['content_type'])}
                    upload_response = requests.post(upload_url, params=params, files=files, timeout=TIMEOUT)
                    
                if upload_response.status_code == 201:
                    print(f"      - Successfully uploaded {asset_name}.")
                    break # Success, break out of the retry loop
                else:
                    print(f"      - Upload attempt {attempt} failed. Status: {upload_response.status_code}", file=sys.stderr)
                    if attempt == MAX_RETRIES:
                        print(f"      - Giving up on uploading {asset_name}. Response: {upload_response.text}", file=sys.stderr)
                        
            except requests.exceptions.RequestException as e:
                print(f"      - Upload attempt {attempt} timed out or dropped: {e}", file=sys.stderr)
                if attempt == MAX_RETRIES:
                    print(f"      - Failed to upload {asset_name} after {MAX_RETRIES} attempts.", file=sys.stderr)
                time.sleep(2 ** attempt) # Wait 2s, then 4s, then retry
